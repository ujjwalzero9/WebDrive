...
function FolderViewer({ userId }) {
  ...
  const fetchData = (folderPath) => {
    axios.post('https://ujjwalzero7.pythonanywhere.com/api/get_entities/', { user_id: userId, folder_path: folderPath })
      .then((response) => {
  ...
  const deleteFolder = async (folderPath) => {
    try {
      ...
      await axios.delete(`https://ujjwalzero7.pythonanywhere.com/api/delete_entity${folderPath}`);
  ...
  const addNewFolder = async () => {
    try {
      ...
      await axios.post('https://ujjwalzero7.pythonanywhere.com/api/create_entity/', {
        user_id: userId,
