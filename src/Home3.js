...
function Home3() {
  ...
  const fetchData = useCallback((folderPath) => {
    axios
      .post('http://127.0.0.1:8000/api/get_entities/', { user_id: userId, folder_path: folderPath })
      .then((response) => {
  ...
  const handleFolderClick = (folderPath) => {
    ...
    setFolderHistory((prevHistory) => [...prevHistory, folderPath]);
  };
  ...
  const handleRenameSave = () => {
    ...
    axios
      .put(`http://127.0.0.1:8000/api/rename_entity/`, {
  ...
  const handleDeleteClick = () => {
    ...
    axios
      .delete(`http://127.0.0.1:8000/api/delete_entity/`, {
  ...
  const openImage = (folder) => {
    if (!folder.is_folder) {
      axios
        .post(`http://127.0.0.1:8000/api/entity_details/`, { entity_id: folder.id })
        .then((response) => {
